...
const HomePage=async(req,res)=>{
    ...
    res.render('AllPosts',{posts:AllPosts,title,login:false})
}
...
const registerValidations =[
    ...
    check('address').isLength({min:6}).withMessage("Write a valid Address"),
    check('phoneNo').isLength({min:10}).withMessage("Enter a valid Phone No"),
]
        ...
        const userEmail = await Users.findOne({email});
            if(userEmail===null){
                ...
                const newUser = new Users({
                    name,email,password:hashed,address,phoneNo
                })
                try{
